...
function YieldBar({ value }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color, minWidth: 40 }}>{value != null ? `${value}%` : "—"}</span>
    </div>
  );
}
...
export function ProductionModule() {
  ...
  useEffect(() => { setSettings(loadModuleSettings("production")); }, []);
  ...
  const totalPassed   = complete.reduce((s, b) => s + b.passed, 0);
  const avgYield      = complete.length > 0 ? (complete.reduce((s, b) => s + b.yield, 0) / complete.length).toFixed(1) : "—";
  ...
  const lineStats = ["Line A", "Line B", "Line C"].map(line => {
    const lb   = complete.filter(b => b.line === line);
    const yVal = lb.length ? (lb.reduce((s, b) => s + b.yield, 0) / lb.length).toFixed(1) : null;
    return { line, batches: lb.length, yield: yVal };
  });
  ...
  return (
    <div style={{ padding: "20px 24px 48px" }}>
      ...
      <HeroBanner
        ...
        subtitle="Batch quality metrics, yield rates, and line-level defect counts across all production runs."
        chips={[
          { label: "Active",    value: String(inProgress.length), sub: "Batches in progress" },
          { label: "Avg Yield", value: `${avgYield}%`,            sub: "Completed batches" },
          { label: "Defects",   value: String(totalDefects),      sub: "All batches combined" },
        ]}
      >
      ...
      {!isConnected && (
        <ConnectBanner
          accent={ACCENT}
          message="Connect a Quickbase report to load live production batch records."
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14, marginBottom: 24 }}>
        <ModuleKpiCard label="Active Batches"  value={inProgress.length} sub="In production now"    accent={C.brand}   />
        <ModuleKpiCard label="Avg Yield Rate"  value={`${avgYield}%`}    sub="Completed batches"    accent={ACCENT}    />
        <ModuleKpiCard label="Units Processed" value={totalUnits}         sub="Across all batches"   accent={C.accent}  />
        <ModuleKpiCard label="Total Defects"   value={totalDefects}       sub="All batches combined" accent={C.danger}  />
        <ModuleKpiCard label="Units Passed"    value={totalPassed}        sub="First-pass quality"   accent={C.success} />
      </div>

      {/* Line stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 24 }}>
        {lineStats.map(({ line, batches: cnt, yield: y }) => (
          <div key={line} style={{ background: C.card, border: `1px solid ${C.borderLight}`, borderRadius: 12, padding: "16px 20px", boxShadow: "0 1px 3px rgba(0,0,0,.06)" }}>
