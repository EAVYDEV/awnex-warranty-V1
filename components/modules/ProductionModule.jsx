...
const C = colors;
const ACCENT = "#D97706"; // module-identity color (header bar, yield data) — intentionally fixed
...
const STATUS_CFG = {
  ...
  "On Hold":     { bg: 'var(--t-warning-soft)',  text: C.warningText, dot: 'var(--t-warning)' },
};
...
function ConnectBanner({ onSettings }) {
  return (
    <div style={{ background: T.brandSubtle, border: `1px dashed ${C.brand}`, borderRadius: T.radiusContainer, padding: "16px 20px", marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
      ...
      <button onClick={onSettings} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: T.radiusInput, fontSize: 12, fontWeight: 600, cursor: "pointer", background: C.brand, border: "none", color: "#fff" }}>
        Connect QB Report
      </button>
    </div>
  );
    ...
    <div style={{ padding: "32px 32px 48px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        ...
        <button
          ...
          style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: T.radiusInput, fontSize: 12, fontWeight: 600, cursor: "pointer", background: C.card, border: `1px solid ${C.borderLight}`, color: C.text2 }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 24 }}>
        {lineStats.map(({ line, batches: cnt, yield: y }) => (
          <div key={line} style={{ background: C.card, border: `1px solid ${C.borderLight}`, borderRadius: T.radiusContainer, padding: "16px 20px", boxShadow: T.cardShadow }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
            {["All", "In Progress", "Complete"].map(f => (
              <button key={f} onClick={() => setActiveFilter(f)} style={{
                ...
                border: `1px solid ${activeFilter === f ? C.brand : C.borderLight}`,
                background: activeFilter === f ? C.brand : C.surface,
            ...
            <tbody>
              {filteredBatches.map((row, i) => (
                <tr key={row.id} style={{ borderBottom: i < filteredBatches.length - 1 ? `1px solid ${C.borderLight}` : "none" }}>
                  <td style={{ padding: "10px 14px", fontWeight: 700, color: C.brand }}>{row.id}</td>
