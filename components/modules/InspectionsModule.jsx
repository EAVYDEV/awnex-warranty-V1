...
const RESULT_CFG = {
  ...
  Rework: { bg: C.warningSoft,   text: C.warningText, dot: C.warning },
};
  ...
  return (
    <div style={{ padding: "32px 32px 48px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14, marginBottom: 28 }}>
        <KpiCard label="Total Inspections" value={total}         sub="All-time sample data" />
        <KpiCard label="Pass Rate"         value={`${passRate}%`} sub={`${passed} of ${total} passed`} />
        <KpiCard label="Failed"            value={failed}         sub="Require rework or scrap" />
        <KpiCard label="Rework"            value={rework}         sub="Conditional pass" />
        <KpiCard label="Total Defects"     value={totalDefects}   sub="Across all inspections" />
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.borderLight}`, borderRadius: 6, padding: "18px 20px", marginBottom: 24, boxShadow: T.cardShadow }}>
        ...
        <div style={{ display: "flex", gap: 0, height: 20, borderRadius: 999, overflow: "hidden", background: C.bg }}>
          ...
          {rework > 0 && <div style={{ flex: rework, background: C.warning }} title={`Rework: ${rework}`} />}
          {failed > 0 && <div style={{ flex: failed, background: C.danger }} title={`Fail: ${failed}`} />}
        </div>
        <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
          {[["Pass", C.success, passed], ["Rework", C.warning, rework], ["Fail", C.danger, failed]].map(([l, color, v]) => (
