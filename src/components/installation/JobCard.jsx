...
export function JobCard({ job, onOpenDetail, onStatusChange }) {
  ...
  return (
    <div style={{ borderRadius: 6, border: `1px solid ${T.borderLight}`, background: T.card, padding: 12, boxShadow: T.cardShadow }}>
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
        <button style={{ borderRadius: 6, background: T.brand, color: T.card, border: "none", padding: "6px 10px", fontSize: 12, fontWeight: 700, cursor: "pointer" }} onClick={onOpenDetail}>Details</button>
        <select style={{ borderRadius: 6, border: `1px solid ${T.borderLight}`, padding: "6px 10px", fontSize: 12, color: T.text1, background: T.card }} value={job.status} onChange={(e) => onStatusChange?.(job, e.target.value)}>
