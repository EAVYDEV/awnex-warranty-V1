// ─── DASHBOARD STORAGE ────────────────────────────────────────────────────────
// localStorage helpers for QB connection settings and dashboard configurations.
// Structured so configs can later be persisted to Quickbase or SharePoint.

import { DEFAULT_KPI_CONFIGS, DEFAULT_CHART_CONFIGS } from "./dashboardDefaults.js";

export const LS_TABLE              = "awntrak_warranty_table_id";
export const LS_REPORT             = "awntrak_warranty_report_id";

// ─── PER-MODULE QB CONNECTION ─────────────────────────────────────────────────

const MODULE_QB_KEYS = {
  warranty:     { table: "awntrak_warranty_table_id",     report: "awntrak_warranty_report_id" },
  installation: { table: "awntrak_installation_table_id", report: "awntrak_installation_report_id" },
  quality:      { table: "awntrak_quality_table_id",      report: "awntrak_quality_report_id" },
  inspections:  { table: "awntrak_inspections_table_id",  report: "awntrak_inspections_report_id" },
  ncrs:         { table: "awntrak_ncrs_table_id",         report: "awntrak_ncrs_report_id" },
  capas:        { table: "awntrak_capas_table_id",        report: "awntrak_capas_report_id" },
  production:   { table: "awntrak_production_table_id",   report: "awntrak_production_report_id" },
};

export function loadModuleSettings(module) {
  const keys = MODULE_QB_KEYS[module] || MODULE_QB_KEYS.warranty;
  try {
    return {
      tableId:  localStorage.getItem(keys.table)  || "",
      reportId: localStorage.getItem(keys.report) || "",
    };
  } catch {
    return { tableId: "", reportId: "" };
  }
}

export function saveModuleSettings(module, { tableId, reportId }) {
  const keys = MODULE_QB_KEYS[module] || MODULE_QB_KEYS.warranty;
  try {
    localStorage.setItem(keys.table,  tableId  ?? "");
    localStorage.setItem(keys.report, reportId ?? "");
  } catch {}
}
export const LS_KPI_CONFIGS        = "awntrak_kpi_configs";
export const LS_CHART_CONFIGS      = "awntrak_chart_configs";
export const LS_COLUMN_TITLES      = "awntrak_column_titles";
export const LS_COLUMN_ORDER       = "awntrak_column_order";
export const LS_DASHBOARD_TITLE    = "awntrak_dashboard_title";
export const LS_DASHBOARD_SUBTITLE = "awntrak_dashboard_subtitle";
export const LS_FILTER_FIELDS      = "awntrak_filter_fields";
export const LS_STICKY_COLUMNS    = "awntrak_sticky_columns";

export const DEFAULT_DASHBOARD_TITLE    = "Quality Management System";
export const DEFAULT_DASHBOARD_SUBTITLE = "Awnex Manufacturing QMS";

// ─── QB CONNECTION ────────────────────────────────────────────────────────────

export function loadConnectionSettings() {
  return loadModuleSettings("warranty");
}

export function saveConnectionSettings({ tableId, reportId }) {
  saveModuleSettings("warranty", { tableId, reportId });
}

// ─── KPI CONFIGS ─────────────────────────────────────────────────────────────

export function loadKpiConfigs() {
  try {
    const raw = localStorage.getItem(LS_KPI_CONFIGS);
    if (!raw) return DEFAULT_KPI_CONFIGS;
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) && parsed.length ? parsed : DEFAULT_KPI_CONFIGS;
  } catch {
    return DEFAULT_KPI_CONFIGS;
  }
}

export function saveKpiConfigs(configs) {
  try {
    localStorage.setItem(LS_KPI_CONFIGS, JSON.stringify(configs));
  } catch {}
}

// ─── CHART CONFIGS ────────────────────────────────────────────────────────────

export function loadChartConfigs() {
  try {
    const raw = localStorage.getItem(LS_CHART_CONFIGS);
    if (!raw) return DEFAULT_CHART_CONFIGS;
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) && parsed.length ? parsed : DEFAULT_CHART_CONFIGS;
  } catch {
    return DEFAULT_CHART_CONFIGS;
  }
}

export function saveChartConfigs(configs) {
  try {
    localStorage.setItem(LS_CHART_CONFIGS, JSON.stringify(configs));
  } catch {}
}

// ─── COLUMN TITLES ────────────────────────────────────────────────────────────

export function loadColumnTitles() {
  try {
    const raw = localStorage.getItem(LS_COLUMN_TITLES);
    return raw ? JSON.parse(raw) : {};
  } catch {
    return {};
  }
}

export function saveColumnTitles(titles) {
  try {
    localStorage.setItem(LS_COLUMN_TITLES, JSON.stringify(titles));
  } catch {}
}

// ─── COLUMN ORDER ─────────────────────────────────────────────────────────────

export function loadColumnOrder() {
  try {
    const raw = localStorage.getItem(LS_COLUMN_ORDER);
    return raw ? JSON.parse(raw) : [];
  } catch {
    return [];
  }
}

export function saveColumnOrder(order) {
  try { localStorage.setItem(LS_COLUMN_ORDER, JSON.stringify(order)); } catch {}
}

// ─── DASHBOARD TITLE / SUBTITLE ───────────────────────────────────────────────

export function loadDashboardTitle() {
  try {
    return localStorage.getItem(LS_DASHBOARD_TITLE) || DEFAULT_DASHBOARD_TITLE;
  } catch {
    return DEFAULT_DASHBOARD_TITLE;
  }
}

export function saveDashboardTitle(title) {
  try { localStorage.setItem(LS_DASHBOARD_TITLE, title); } catch {}
}

export function loadDashboardSubtitle() {
  try {
    return localStorage.getItem(LS_DASHBOARD_SUBTITLE) || DEFAULT_DASHBOARD_SUBTITLE;
  } catch {
    return DEFAULT_DASHBOARD_SUBTITLE;
  }
}

export function saveDashboardSubtitle(subtitle) {
  try { localStorage.setItem(LS_DASHBOARD_SUBTITLE, subtitle); } catch {}
}

// ─── FILTER FIELD SELECTION ──────────────────────────────────────────────────

export function loadFilterFields() {
  try {
    const raw = localStorage.getItem(LS_FILTER_FIELDS);
    const parsed = raw ? JSON.parse(raw) : [];
    return Array.isArray(parsed) ? parsed : [];
  } catch {
    return [];
  }
}

export function saveFilterFields(fields) {
  try { localStorage.setItem(LS_FILTER_FIELDS, JSON.stringify(fields)); } catch {}
}

// ─── STICKY COLUMNS ──────────────────────────────────────────────────────────

export function loadStickyColumns() {
  try {
    const raw = localStorage.getItem(LS_STICKY_COLUMNS);
    return new Set(raw ? JSON.parse(raw) : []);
  } catch {
    return new Set();
  }
}

export function saveStickyColumns(cols) {
  try {
    localStorage.setItem(LS_STICKY_COLUMNS, JSON.stringify([...cols]));
  } catch {}
}

// ─── RESET ────────────────────────────────────────────────────────────────────

export function resetAllConfigs() {
  try {
    localStorage.removeItem(LS_KPI_CONFIGS);
    localStorage.removeItem(LS_CHART_CONFIGS);
    localStorage.removeItem(LS_COLUMN_TITLES);
    localStorage.removeItem(LS_COLUMN_ORDER);
    localStorage.removeItem(LS_DASHBOARD_TITLE);
    localStorage.removeItem(LS_DASHBOARD_SUBTITLE);
    localStorage.removeItem(LS_FILTER_FIELDS);
    localStorage.removeItem(LS_STICKY_COLUMNS);
    localStorage.removeItem("awntrak_alert_thresholds");
    localStorage.removeItem("awntrak_watched_orders");
    localStorage.removeItem("awntrak_risk_history");
  } catch {}
  return {
    kpiConfigs:   DEFAULT_KPI_CONFIGS,
    chartConfigs: DEFAULT_CHART_CONFIGS,
  };
}

// Clears all stored data including QB connection and geocache — full fresh start.
export function clearAllData() {
  try {
    localStorage.removeItem(LS_TABLE);
    localStorage.removeItem(LS_REPORT);
    localStorage.removeItem("awntrak_installation_table_id");
    localStorage.removeItem("awntrak_installation_report_id");
    localStorage.removeItem("awntrak_quality_table_id");
    localStorage.removeItem("awntrak_quality_report_id");
    localStorage.removeItem("awntrak_inspections_table_id");
    localStorage.removeItem("awntrak_inspections_report_id");
    localStorage.removeItem("awntrak_ncrs_table_id");
    localStorage.removeItem("awntrak_ncrs_report_id");
    localStorage.removeItem("awntrak_capas_table_id");
    localStorage.removeItem("awntrak_capas_report_id");
    localStorage.removeItem("awntrak_production_table_id");
    localStorage.removeItem("awntrak_production_report_id");
    localStorage.removeItem(LS_KPI_CONFIGS);
    localStorage.removeItem(LS_CHART_CONFIGS);
    localStorage.removeItem(LS_COLUMN_TITLES);
    localStorage.removeItem(LS_COLUMN_ORDER);
    localStorage.removeItem(LS_DASHBOARD_TITLE);
    localStorage.removeItem(LS_DASHBOARD_SUBTITLE);
    localStorage.removeItem(LS_FILTER_FIELDS);
    localStorage.removeItem(LS_STICKY_COLUMNS);
    localStorage.removeItem("awntrak_geocache");
    localStorage.removeItem("awntrak_alert_thresholds");
    localStorage.removeItem("awntrak_watched_orders");
    localStorage.removeItem("awntrak_risk_history");
  } catch {}
}
