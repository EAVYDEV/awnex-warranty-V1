...
export function mapInstallationData(payload) {
  ...
  return payload.data.map((record, idx) => {
    ...
    return {
      ...
      projectManager: (() => {
        const pmRaw = pick(record, labelToId, FIELD_ALIASES.ProjectManager);
        if (pmRaw && typeof pmRaw === "object") {
          const candidate = pmRaw.name || pmRaw.userName || pmRaw.email || pmRaw.label || pmRaw.value;
          if (candidate) return String(candidate);
          const primitive = Object.values(pmRaw).find((v) => typeof v === "string" && v.trim());
          return primitive ? String(primitive) : "Unassigned";
        }
        return String(pmRaw || "Unassigned");
      })(),
