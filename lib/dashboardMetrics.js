...
export function applyFilter(records, condition) {
  ...
  return records.filter(r => {
    ...
    switch (op) {
      ...
      case "isempty":       return v === null || v === undefined || v === "" || v === 0;
      case "isnotempty":    return v !== null && v !== undefined && v !== "" && v !== 0;
      default:              return true;
    }
  });
}
...
export function aggregateField(records, field, type) {
  ...
  switch (type) {
    ...
    case "min": return Math.min(...records.map(r => parseFloat(r[field]) || 0));
    case "max": return Math.max(...records.map(r => parseFloat(r[field]) || 0));
    default:    return 0;
  }
}
